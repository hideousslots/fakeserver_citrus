...
export class LineWinCalculator {
	...
	public calculateLineWins(
		reels: CitrusGotReelSymbol[][],
		bet: number
	): LineWinDetails[] {
		...
		this.reels = reels;
		const wins: LineWinDetails[] = this.lineDefinitions
			.map((line, index) => this.checkLineWin(line, bet, index))
			.filter(Boolean);
	...
	private checkLineWin = (
		...
		let overallMultiplier =
			"multiplier" in lineSymbol && lineSymbol.multiplier > 1
				? lineSymbol.multiplier
				: 0;
		...
		for (let i = 1; i < lineDefinition.length; i++) {
			...
			if (isLineSymbolSet) {
				if (
					currentSymbol.symbol !== lineSymbol.symbol &&
					!this.isWildSymbol(currentSymbol.symbol)
				) {
					break;
				}
		...
		if (consecutiveWildCount >= 3) {
			const asWilds = this.lookupPayTable(
	...
	private lookupPayTable(
		...
		return (
			payTableEntry[index] / this.currentMathConfig.coinsPerBet_main || 0
		);
	}
}
