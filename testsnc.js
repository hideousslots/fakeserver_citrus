...
const gameInterface = __importDefault(require("./lib/games/space-hive-7/index.js"));
...
app.post('/game/play', async (req, res) => {
    ...
    // console.log('stored response: ' + JSON.stringify(gameResponse));

    // console.log('\nplay response: ' + JSON.stringify(await gameInterface.play(0.5)));

    //Check for any wins

    // for(let i=0;i<1000;i++) {
    //     const gameResponse = await gameInterface.play(1,"main");

    // }

    const gameResponse = gameInterface.index.play({ bet: 1, action: "main", state: null, variant: null, promo: null });
    gameResponse.state = {};
    ...
    if (response.wager.win !== 0) {
        console.log('winning response to play is ' + JSON.stringify(response));
    }
    res.send(JSON.stringify(response));
...
const startup = async () => {
    ...
    console.log('Test result...' + JSON.stringify(gameInterface.index.play({ bet: 1, action: "main", state: null, variant: null, promo: null })));
